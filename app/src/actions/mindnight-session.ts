...
export async function getClient(){
  ...
  let client = await database.client.createOrFind({data:{uuid:cachedUUID, settings:{}}, include:{mindnight_session:true}}, {where:{uuid:cachedUUID}, include:{mindnight_session:true}}); //await database.client.findFirst({where:{}, include:{mindnight_session:true}});
  return client;
}
